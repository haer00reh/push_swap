...
/*   Updated: 2025/12/26 16:48:28 by haer-reh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	combined_rotations(t_stack *a, t_stack *b, int *pos_b, 
							int *target_pos, int size_a, int size_b)
{
	int	rotate_b_forward;
	int	rotate_a_forward;

	rotate_b_forward = (*pos_b <= size_b / 2);
	rotate_a_forward = (*target_pos <= size_a / 2);
	...
	{
		while (*pos_b < size_b && *target_pos < size_a)
		{
			rrr(a, b);
			(*pos_b)++;
			(*target_pos)++;
...
static void	execute_move(t_stack *a, t_stack *b, int pos_b, int target_pos)
{
	...
	size_a = a->size;
	size_b = b->size;
	combined_rotations(a, b, &pos_b, &target_pos, size_a, size_b);
	finish_rotations(a, b, pos_b, target_pos, size_a, size_b);
	pa(a, b);
}
...
{
	int	(lis_len), (rotations), (tp), (median);
